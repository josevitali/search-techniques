...
dfsSearch :: (Eq a, Problem a) => a -> Maybe (Int, Int, [a])
...
iddfsSearch :: (Eq a, Problem a) => a -> Maybe (Int, Int, [a])
iddfsSearch startState = solveIddfs 0 startState

solveIddfs :: (Eq a, Problem a) => Float -> a -> Maybe (Int, Int, [a])
...
solve opened visited maxDepth
            ...
            | any (\(cost, node) -> node == nodeState && cost <= nodeCost) visited = solve popedOpened visited maxDepth
            -- if max depth reached skip and keep searching
            | nodeDepth > maxDepth = solve popedOpened visited maxDepth
            -- if current state is Goal, return found path
            | isGoal nodeState = Just (length visited, nodeCost, nodeToList currentNode)
            -- otherwise explode current state and add child states to open nodes
            | otherwise = solve nextOpened ((nodeCost, nodeState):visited) maxDepth
                    where 
